...
export class CalculadoraComponent implements OnInit {
  ...
  /**
   * recebe uma operacao,e chama o metodo calcular
   * e atualiza tudo
   * @param operacao 
   * @returns 
   */
 ...
 */
  calcular():void{
    ...
    //muda de string pra float para os calculos
    this.resultado = this.calculadoraService.calcular(
      parseFloat(this.numero1),
      parseFloat(this.numero2),
      this.operacao
    ).toString();
    this.numero1= this.resultado;//Lucas sugeriu colocar essas 2 linhas
    this.operacao ='null';
    this.numero2 ='null';
  }
  ...
  get display(): string{
    if(this.resultado !='null'){
      return this.resultado.toString();//converte para display
    }
